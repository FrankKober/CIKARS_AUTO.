...
export default function Navbar() {
  ...
  return (
    <>
      <nav ref={menuRef} className="fixed top-0 left-0 right-0 z-50 border-b border-white/10 bg-black/70 backdrop-blur-xl transition-all duration-300">
        ...
        {/* Mobile Dropdown */}
        {mobileMenuOpen && (
          <div className="md:hidden absolute top-20 left-0 right-0 bg-black/95 border-b border-neutral-900 px-6 py-6 flex flex-col gap-5 backdrop-blur-2xl shadow-2xl">
            <Link href="/cars" onClick={() => setMobileMenuOpen(false)} className="text-base font-medium text-neutral-300 hover:text-white transition">Browse</Link>
            <button 
              onClick={handleSellClick} 
            ...
            <Link href="/about" onClick={() => setMobileMenuOpen(false)} className="text-base font-medium text-neutral-300 hover:text-white transition">About</Link>
            <Link href="/careers" onClick={() => setMobileMenuOpen(false)} className="text-base font-medium text-neutral-300 hover:text-white transition">Careers</Link>
            <Link href="/contact" onClick={() => setMobileMenuOpen(false)} className="text-base font-medium text-neutral-300 hover:text-white transition">Contact</Link>
            <Link href="/blog" onClick={() => setMobileMenuOpen(false)} className="text-base font-medium text-neutral-300 hover:text-white transition">Blog</Link>
            ...
            <div className="pt-4 border-t border-neutral-800 flex flex-col gap-3">
              {isAuthenticated ? (
                <>
                  ...
                  <button 
                    onClick={handleLogout}
                    className="w-full text-center py-3 rounded-full bg-neutral-900 text-red-400 text-sm font-bold border border-neutral-800 transition cursor-pointer"
                  >
                    Log Out
                  </button>
                </>
